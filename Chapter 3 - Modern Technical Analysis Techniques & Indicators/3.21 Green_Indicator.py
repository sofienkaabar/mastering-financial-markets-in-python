...
from master_library import import_data, ohlc_plot, green_indicator
...
def custom_signal_chart(my_time_series, window, choice='bars', source='open', chart_type='ohlc'): 
    ...
    bearish_signal = Line2D([0], [0], marker='v', color='w', label='Sell signal', markerfacecolor='green', markersize=10)
    plt.legend(handles=[bullish_signal, bearish_signal]) 
    
# download the historical values
my_time_series = import_data('NKE')
# call the function(s)
my_time_series = green_indicator(my_time_series)
# plot
