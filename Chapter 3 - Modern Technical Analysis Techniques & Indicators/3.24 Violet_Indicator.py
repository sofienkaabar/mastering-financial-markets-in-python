...
from master_library import import_data, ohlc_plot, violet_indicator
...
def custom_signal_chart(my_time_series, window, choice='bars', source='open', chart_type='ohlc'): 
    ...
    bullish_signal = Line2D([0], [0], marker='^', color='w', label='Buy signal', markerfacecolor='violet', markersize=10)
    bearish_signal = Line2D([0], [0], marker='v', color='w', label='Sell signal', markerfacecolor='violet', markersize=10)
    plt.legend(handles=[bullish_signal, bearish_signal])  

# download the historical values
my_time_series = import_data('DIS')
# call the function(s)
my_time_series = violet_indicator(my_time_series)
# plot
custom_signal_chart(my_time_series, 1000, choice='bars', source='open', chart_type='ohlc')