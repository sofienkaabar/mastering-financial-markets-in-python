...
from master_library import import_data, bollinger_bands, signal_chart, moving_average

def bb_trend_friendly(my_time_series, bb_lookback=20, ma_lookback=200, num_std_dev=2):
    my_time_series = bollinger_bands(my_time_series, source='close', bb_lookback=bb_lookback, num_std_dev=num_std_dev)
    my_time_series = moving_average(my_time_series, source='close', ma_lookback=ma_lookback, output_name='moving_average', ma_type='SMA')
...
my_time_series = import_data('PEP')
# call the function(s)
my_time_series = bb_trend_friendly(my_time_series, bb_lookback=20, num_std_dev=2)
# plot
signal_chart(my_time_series, 500)
plt.plot(my_time_series['lower_band'].iloc[-500:], label='Lower Band', color='darkgrey')
plt.plot(my_time_series['upper_band'].iloc[-500:], label='Upper Band', color='black')
plt.plot(my_time_series['moving_average'].iloc[-500:], color='blue', linestyle='dashed')