...
from master_library import import_data, ohlc_plot, orange_indicator
...
def custom_signal_chart(my_time_series, window, choice='bars', source='open', chart_type='ohlc'): 
    ...
    bullish_signal = Line2D([0], [0], marker='^', color='w', label='Buy signal', markerfacecolor='orange', markersize=10)
    bearish_signal = Line2D([0], [0], marker='v', color='w', label='Sell signal', markerfacecolor='orange', markersize=10)
    plt.legend(handles=[bullish_signal, bearish_signal]) 
                
# download the historical values
my_time_series = import_data('TSLA')
# call the function(s)
my_time_series = orange_indicator(my_time_series)
# plot
